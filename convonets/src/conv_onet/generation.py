...
import trimesh
from convonets.src.utils import libmcubes
from convonets.src.common import make_3d_grid, normalize_coord, add_key, coord2index
from convonets.src.utils.libsimplify import simplify_mesh
from convonets.src.utils.libmise import MISE
import time
import math
...
class Generator3D(object):
    ...
    def eval_occupancy_points(self, points):
        """ Evaluates the occupancy values for the points, assuming we have a latent code.
        ...
        """
        self.model.eval()
        points_splits = torch.split(points, self.points_batch_size)
        occ_hats = []
        for points_subset in points_splits:
            points_subset = points_subset.unsqueeze(0).to(self.device)
            with torch.no_grad():
                occ_hat = self.model.decode(points_subset, self.latent_code).probs
            occ_hats.append(occ_hat.squeeze(0).detach().cpu())

        occ_hat = torch.cat(occ_hats, dim=0)
        return occ_hat

    def eval_occupancy_for_refinement(self, points):
        """ Will compute gradients and keep everything on one device.

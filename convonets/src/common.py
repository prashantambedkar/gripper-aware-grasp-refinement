...
def sdf_to_occ(sdf_values):
    """
    Converts the sdf values (output of tanh) in [-1, 1] to occupancy [0, 1].
    Inverts and shifts the data to fit into [0, 1]

    negative sdf -1 ==> occ 1
    positive sdf +1 ==> occ 0

    Args:
        sdf_values (torch.Tensor): array of sdf values

    Returns:
        torch.Tensor: array of occupancy values
    """
    occupancy = (1 - sdf_values) / 2
    return occupancy
